			...
			PushModeControllerI
{
		...
		) throws Exception
	{
		...
		// the scheduled one is used to schedule the pushes of dynamic data
		// when requested.
		super(1, 1) ;
	...
	// ------------------------------------------------------------------------
	// Component introspection services (ComputerDynamicStateDataI)
	// ------------------------------------------------------------------------
	...
	/**
	 * start the pushing of computer dynamic state information and force the
	...
	/**
	 * start <code>n</code> pushing of computer dynamic state information and
		...
		this.pushingFuture =
			this.scheduleTask(
					new ComponentI.ComponentTask() {
						...
						public void run() {
							try {
								c.sendDynamicState(fInterval, fN) ;
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
